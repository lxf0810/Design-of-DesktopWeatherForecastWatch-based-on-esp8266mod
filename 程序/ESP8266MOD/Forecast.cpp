...
void Forecast::_parseForecastInfo(WiFiClient httpClient){
  ...
  // 从以上信息中解析数值
  _date[0]=results_0_daily_0["date"].as<String>(); 
  ...
  _date[1]=results_0_daily_1["date"].as<String>(); 
  ...
  _date[2]=results_0_daily_2["date"].as<String>(); 
...
// 返回白天天气（字符串格式）
String Forecast::getDate(int index){
  return _date[index];
}
